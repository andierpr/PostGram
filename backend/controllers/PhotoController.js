...
const updatePhoto = async (req, res) => {
  ...
  // Check if photo exists (se foto existe)
  if (!photo) {
    res.status(404).json({ errors: ["Foto não encontrada!"] });
    return;
  }
...
//Como funcionalidade // Like functionality 
const likePhoto = async (req, res) => {
  ...
  //Verifique se o usuário já gostou da foto
  if (photo.likes.includes(reqUser._id)) {
    res.status(422).json({ errors: ["Você já curtiu esta foto."] });
    return;
  }
  //falta criar um disliked


  //

  // Put user id in array of likes
  //Coloque o ID do usuário em uma matriz de curtidas
...
const commentPhoto = async (req, res) => {
  ...
  //Coloque o comentário na matriz de comentários
  const userComment = {
    comment,
    userName: user.name,
...
//Pesquisar uma foto pelo título
